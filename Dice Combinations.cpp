...
int combinations(int n){
    ...
    if(dp[n]!=-1){
        ...
        return dp[n] = (combinations(n-1)%mod+ combinations(n-2)%mod +combinations(n-3)%mod + combinations(n-4)%mod+ combinations(n-5)%mod + combinations(n-6)%mod)%mod;
    }
}
